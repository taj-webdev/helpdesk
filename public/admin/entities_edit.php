<?php
// public/admin/entities_edit.php
session_start();
...
// SweetAlert dari redirect (optional)
...
// Default value dari DB / POST
$unit_id       = $_POST['unit_id']       ?? $entity['unit_id'];
$nama_pengguna = $_POST['nama_pengguna'] ?? $entity['nama_pengguna'];
$nama_entitas  = $_POST['nama_entitas']  ?? $entity['nama_entitas'];
$serial_number = $_POST['serial_number'] ?? $entity['serial_number'];
$tipe_entitas  = $_POST['tipe_entitas']  ?? $entity['tipe_entitas'];
$brand         = $_POST['brand']         ?? $entity['brand'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($unit_id <= 0 || $nama_entitas === '') {
        ...
        try {
            ...
            $upd->bindValue(':unit_id', $unit_id, PDO::PARAM_INT);
            $upd->bindValue(':nama_pengguna', $nama_pengguna !== '' ? $nama_pengguna : null, PDO::PARAM_STR);
            $upd->bindValue(':nama_entitas', $nama_entitas, PDO::PARAM_STR);
            $upd->bindValue(':serial_number', $serial_number !== '' ? $serial_number : null, PDO::PARAM_STR);
            $upd->bindValue(':tipe_entitas', $tipe_entitas !== '' ? $tipe_entitas : null, PDO::PARAM_STR);
            $upd->bindValue(':brand', $brand !== '' ? $brand : null, PDO::PARAM_STR);
            $upd->bindValue(':id', $id, PDO::PARAM_INT);

            $upd->execute();

            header('Location: entities.php?status=success&message=' . urlencode('Perubahan entity berhasil disimpan.'));
            exit;
        } catch (PDOException $e) {
            $localError = 'Gagal memperbarui data entity: ' . $e->getMessage();
        }
    }
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>Edit Entity - Helpdesk NIP</title>
    ...
    <div class="flex-1 flex flex-col">
        ...
        <main class="flex-1 px-4 md:px-6 lg:px-8 py-6 space-y-6 fade-in-soft-delayed">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div>
                    <h1 class="text-xl md:text-2xl font-semibold">Edit Entity</h1>
                    <p class="text-xs md:text-sm text-slate-500">
                        Ubah informasi entitas: <?= htmlspecialchars($entity['nama_entitas'], ENT_QUOTES, 'UTF-8'); ?>
                    </p>
                </div>
                <a href="entities.php"
            ...
            <section class="bg-white rounded-3xl shadow-lg border border-slate-200 p-4 md:p-6 fade-in-soft">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        <h2 class="text-sm md:text-base font-semibold text-slate-800">Form Edit Entity</h2>
                        <p class="text-xs md:text-sm text-slate-500">
                            Perbarui data entitas sesuai kebutuhan.
                        </p>
                    </div>
                </div>
