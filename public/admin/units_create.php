...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // tat_target opsional tapi kalau diisi harus angka >= 0
    $tat_target = null;
    if ($tat_target_raw !== '') {
    ...
    try {
        if ($nama_unit !== '') {
            $stmt = $pdo->prepare("SELECT id FROM units WHERE nama_unit = :nama_unit LIMIT 1");
            $stmt->execute([':nama_unit' => $nama_unit]);
            if ($stmt->fetch()) {
                $errors[] = 'Nama unit sudah digunakan.';
            }
        }

        // Cek unik unit_id (jika diisi)
        if ($unit_id !== '') {
            $stmt = $pdo->prepare("SELECT id FROM units WHERE unit_id = :unit_id LIMIT 1");
            $stmt->execute([':unit_id' => $unit_id]);
            if ($stmt->fetch()) {
                $errors[] = 'Unit ID sudah digunakan.';
            }
        }

        // Kalau tidak ada error → insert
        if (empty($errors)) {
            $sql = "INSERT INTO units (unit_id, nama_unit, alamat, kab_kota, provinsi, tat_target)
                    VALUES (:unit_id, :nama_unit, :alamat, :kab_kota, :provinsi, :tat_target)";
            ...
            header('Location: units.php?status=success&message=' . urlencode('Unit berhasil ditambahkan.'));
            exit;
        }
    } catch (PDOException $e) {
        $errors[] = 'Terjadi kesalahan pada server saat menyimpan data.';
    }
}
?>
<!DOCTYPE html>
...
<head>
    <meta charset="UTF-8">
    <title>Tambah Unit - Helpdesk NIP</title>
        ...
        <main class="flex-1 px-4 md:px-6 lg:px-8 py-6 fade-in-soft-delayed">
            <div class="max-w-3xl mx-auto space-y-5">
                ...
                <div class="flex items-center justify-between gap-3">
                    <div>
                        <h1 class="text-xl md:text-2xl font-semibold">Tambah Unit</h1>
                        <p class="text-xs md:text-sm text-slate-500">
                            Tambahkan unit baru yang akan terhubung dengan entitas dan tiket.
                        </p>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-3xl shadow-lg border border-slate-200 p-5 md:p-6 fade-in-soft">
                    <div class="flex items-center gap-3 mb-4">
                        <div class="inline-flex items-center justify-center w-10 h-10 rounded-2xl bg-indigo-50 text-indigo-600">
                            <!-- icon building/plus -->
                        ...
                        <div>
                            <p class="text-sm font-semibold text-slate-800">Form Unit</p>
                            <p class="text-[11px] text-slate-500">Lengkapi data unit dengan informasi yang valid.</p>
                        </div>
                    </div>
                    ...
                    <form method="post" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="space-y-1.5">
                                <label for="unit_id" class="text-xs font-medium text-slate-700">
                                    Unit ID <span class="text-[10px] text-slate-400">(opsional, bisa dikosongkan)</span>
                                </label>
                                <input
                                    type="text"
                                    id="unit_id"
                            ...
                            <div class="space-y-1.5">
                                ...
                                <input
                                    ...
                                    class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-xs md:text-sm
                                           outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500
                                           placeholder:text-slate-400"
                                    placeholder="Misal: Kantor Cabang Jakarta Selatan"
                                >
                            </div>
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <textarea
                                ...
                                class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-xs md:text-sm
                                       outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500
                                       placeholder:text-slate-400"
                                placeholder="Alamat lengkap unit..."><?= htmlspecialchars($alamat, ENT_QUOTES, 'UTF-8'); ?></textarea>
                        </div>
                        ...
                        <div class="mt-4 flex flex-col sm:flex-row justify-end gap-2">
                            ...
                            <button type="submit"
                                    class="inline-flex items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-4 py-2
                                           text-xs md:text-sm font-semibold text-white shadow-lg hover:bg-indigo-500
